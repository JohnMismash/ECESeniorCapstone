...
void Top_LimitSwitch_Init(void);
	  ...
	  HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_8); //Orange is waiting
		...
		if(GPIOA->IDR & (1<<1)) { //Read PA1 value for top limit switch
			...
			if (lifting) {
				...
				//Pause to allow the motor to stop. Also help prevent continued lifting.
				HAL_Delay(1000);
				
				//We are not lifting or lowering
				lifting = 0;
				lowering = 0;
				...
				//NOW Tell the top board it can detect packages again
				TransmitToTopBoard(READY);
			}			
		}
...
void Top_LimitSwitch_Init(void){
  ...
  EXTI -> RTSR |= (1 << 1);	     	 // ENABLES RISING TRIGGER EVENT FOR PA1.
	EXTI->FTSR &= ~(1<<1); 						//DISABLE FALLING EDGE TRIGGER FOR PA1
	SYSCFG->EXTICR[0] &= ~(0xf << 4);  // ROUTE PA1 TO THE EXTI INPUT LINE 1 (EXTI1). Clear bits [7:4] of EXTICR register to select PA0.
  NVIC_EnableIRQ(EXTI0_1_IRQn);    // ENABLE THE EXTI INTERRUPT. (Enables line 0 and line 1)	
															*/
}
