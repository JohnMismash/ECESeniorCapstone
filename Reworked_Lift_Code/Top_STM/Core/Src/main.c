...
1. Black wire goes to PA1 ON THE BOTTOM BOARD (It is NOT grounded!)
...
int main(void)
{
  ...
  /* Initialize all configured peripherals */
	LED_Init();
	USART_Init();
  //LimitSwitch_Init(); //Moved to bottom board. Only using power from top board.
  ...
  while (1){
   ...
   //range on readVal is aprox between 500 and 3000
		readVal = ADC1->DR; 
		if(readVal > 2000 && READY_TO_GO){ //Only start counting once we are ready to go.
      count++;
			GPIOC->ODR |= (1<<9);  //green
    }
		else{
...
void ReceiveFromBottomBoard(uint8_t read_data){
	//Allow distance sensor to detect packages again
	if(read_data == READY){
		READY_TO_GO = 1;
	}
}
